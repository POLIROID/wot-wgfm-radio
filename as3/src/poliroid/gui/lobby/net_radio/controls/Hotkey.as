	...
	public class Hotkey extends SoundButtonEx implements ISoundButtonEx 
	{
		...
		override protected function onMouseDownHandler(e:MouseEvent) : void
		{
			if (App.utils.commons.isRightButton(e)) 
			{
				...
				{
					dispatchEvent(new NetRadioValueEvent(NetRadioValueEvent.HOTKEY_CHANGED, model.name, COMMAND_STOP_ACCEPT));
				}
				App.contextMenuMgr.show('NetRadioHotkeyContextHandler', this, {'controlName': model.name});
			}
			
			if (App.utils.commons.isLeftButton(e) && !model.isAccepting) 
			{
				dispatchEvent(new NetRadioValueEvent(NetRadioValueEvent.HOTKEY_CHANGED, model.name, COMMAND_START_ACCEPT));
			}
		}
	}
}
